...
alter table CT_PHIEUSUACHUA add NgayLapPhieuSuaChua datetime

alter table CT_PHIEUSUACHUA drop column NgayLapPhieuSuaChua 
...
select XE.Bienso, KHACHHANG.TenKH from XE, KHACHHANG where XE.MaKH=KHACHHANG.MaKH
...
AS
BEGIN
	SELECT XE.BienSo as 'Biển số', TenHieuXe as 'Tên hiệu xe', TenKH as 'Tên khách hàng', MaPhieuSuaChua as 'Mã phiếu sửa chữa hiện có', TongTien as 'Tổng tiền phiếu sửa chữa', TienNo as 'Tổng tiền nợ hiện tại'
	FROM XE, HIEUXE as HX, KHACHHANG as KH, PHIEUSUACHUA
	WHERE XE.MaHX = HX.MaHX and KH.MaKH = XE.MaKH and ((XE.BienSo LIKE '%'+@DuLieu+'%') or (TenHieuXe LIKE '%'+@DuLieu+'%') or (TenKH LIKE '%'+@DuLieu+'%') or (TienNo LIKE '%'+@DuLieu+'%'))
END
GO
